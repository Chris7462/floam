...
void OdomEstimationClass::pointAssociateToMap(const pcl::PointXYZI *const pi, pcl::PointXYZI *const po)
{
...
{
  for (size_t i = 0; i < downsampledEdgeCloud->points.size(); i++) {
    pcl::PointXYZI point_temp;
    pointAssociateToMap(&downsampledEdgeCloud->points[i], &point_temp);
    laserCloudCornerMap->push_back(point_temp);
  }

  for (size_t i = 0; i < downsampledSurfCloud->points.size(); i++) {
    pcl::PointXYZI point_temp;
